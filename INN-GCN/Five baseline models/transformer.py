...
MODEL_SAVE_DIR = r"D:\project\contrast model\cocrystal_prediction_Transformer"
...
def main():
    ...
    input_dim = folds[0]['x_tr'].shape[1]
    ...
    def objective(args):
        nonlocal hp_eval_count
        hp_eval_count += 1
        print(f"\n=============================================")
        print(f"🔍 超参优化迭代: {hp_eval_count}/{MAX_EVALS}")
        print(f"📌 当前参数: {args}")
        print(f"=============================================")

        val_f1_list = []
        for fold_idx, fold in enumerate(folds):
            print(f"\n📂 训练 Fold [{fold_idx + 1}/{N_FOLDS}]")

            x_tr, y_tr = fold['x_tr'], fold['y_tr']
            x_va, y_va = fold['x_va'], fold['y_va']
            ...
            for ep in range(EPOCHS):
                train_loss = train_epoch(model, dl_tr, optimizer, criterion)
                va_pred, va_true = eval_epoch(model, dl_va)
                f1 = f1_score(va_true, (va_pred > 0.5).astype(int), zero_division=0)

                print(f"    Epoch {ep + 1:2d} | loss={train_loss:.4f} | val_f1={f1:.3f}")
                ...
                if f1 > best_f1:
                    ...
                    if patience >= EARLY_STOP_PATIENCE:
                        print(f"    ⏹️  超参搜索早停 | Epoch {ep+1}")
                        break

            print(f"    ✅ Fold {fold_idx + 1} 最佳 F1 = {best_f1:.3f}")
            val_f1_list.append(best_f1)
            del model, optimizer

        mean_f1 = np.mean(val_f1_list)
        print(f"\n🎯 本次迭代平均 F1 = {mean_f1:.4f}")
    ...
    best_params = {
        'd_model': [64, 128, 256][best['d_model']],
        'nhead': [2, 4, 8][best['nhead']],
    ...
    for fold_idx, fold in enumerate(folds):
        print(f"\n=============================================")
        print(f"🚀 最终模型训练 Fold [{fold_idx + 1}/{N_FOLDS}]")
        print(f"=============================================")
        ...
        for ep in range(EPOCHS):
            train_loss = train_epoch(model, dl_tr, optimizer, criterion)
            va_pred, va_true = eval_epoch(model, dl_va)
            tr_pred, tr_true = eval_epoch(model, dl_tr)
            va_met = get_metrics(va_true, va_pred)
            tr_met = get_metrics(tr_true, tr_pred)

            print(
                f"    Epoch {ep + 1:2d} | loss={train_loss:.4f} | val_f1={va_met['f1']:.3f} | val_auc={va_met['auc']:.3f}")

            if va_met['f1'] > best_val_f1:
                best_val_f1 = va_met['f1']
                best_model = model.state_dict()
                patience = 0
            else:
                patience += 1
                if patience >= EARLY_STOP_PATIENCE:
                    print(f"    ⏹️  早停 | Epoch {ep + 1}")
                    break
        ...
        oof_true.extend(va_true)

        print(f"\n✅ Fold {fold_idx + 1} 完成")
        print(f"    Train F1={tr_met['f1']:.3f} | Val F1={va_met['f1']:.3f} | Val AUC={va_met['auc']:.3f}")

    avg_train = {k: np.mean([m[k] for m in train_metrics_list]) for k in
    ...
    joblib.dump(ens, f'{MODEL_SAVE_DIR}/ensemble_transformer_final.pkl')
