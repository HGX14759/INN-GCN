...
class StandardGraphPoolLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        pooled = self.global_avg_pool(inputs)
        ...
        if AUG_SWAP:
            ...
            for idx in swap_indices:
                ...
                swapped_pair_desc = calculate_complementary_features(
                    swapped_api_desc, swapped_ccf_desc, swapped_api_hba_hbd, swapped_ccf_hba_hbd
                )
...
def build_model(hp_params):
    api_atom_input = tf.keras.Input(shape=(MAX_ATOM_NUM, 44), name='api_atom_input')
    api_adj_input = tf.keras.Input(shape=(MAX_ATOM_NUM, MAX_ATOM_NUM), name='api_adj_input')
    api_edge_input = tf.keras.Input(shape=(MAX_ATOM_NUM, MAX_ATOM_NUM, 4), name='api_edge_input')
    ccf_atom_input = tf.keras.Input(shape=(MAX_ATOM_NUM, 44), name='ccf_atom_input')
    ccf_adj_input = tf.keras.Input(shape=(MAX_ATOM_NUM, MAX_ATOM_NUM), name='ccf_adj_input')
    ccf_edge_input = tf.keras.Input(shape=(MAX_ATOM_NUM, MAX_ATOM_NUM, 4), name='ccf_edge_input')
    extra_input = tf.keras.Input(shape=(EXTRA_FEAT_SIZE,), name='extra_input')

    api_inn_hybrid1 = StandardGCNStackLayer(
    ...
    api_inn_hybrid2 = StandardGCNStackLayer(
    ...
    ccf_inn_hybrid1 = StandardGCNStackLayer(
        hp_params['gcn_layer1_size'],
        activation='relu',
        dropout_rate=hp_params.get('dropout_rate', 0.2),
        gate_type='adaptive',
    ...
    ccf_inn_hybrid2 = StandardGCNStackLayer(
        hp_params['gcn_layer2_size'],
        activation='relu',
        dropout_rate=hp_params.get('dropout_rate', 0.2),
        gate_type='adaptive',
    ...
    api_inn_pool = StandardGraphPoolLayer(
    ...
    ccf_inn_pool = StandardGraphPoolLayer(
        units=hp_params['gcn_layer2_size'],
        name='ccf_inn_pool'
    )(ccf_inn2)

    api_feat_enhanced = api_inn_pool
    ccf_feat_enhanced = ccf_inn_pool

    inn_cross_attn = StandardMolCrossLayer(
...
def main():
    ...
    train_metrics_avg = {
        'accuracy': np.mean([m['accuracy'] for m in fold_train_metrics]),
        'precision': np.mean([m['precision'] for m in fold_train_metrics]),
        'recall': np.mean([m['recall'] for m in fold_train_metrics]),
        'f1': np.mean([m['f1'] for m in fold_train_metrics]),
        'auc': np.mean([m['auc'] for m in fold_train_metrics])
    }
    val_metrics_avg = {
        'accuracy': np.mean([m['accuracy'] for m in fold_val_metrics]),
        'precision': np.mean([m['precision'] for m in fold_val_metrics]),
        'recall': np.mean([m['recall'] for m in fold_val_metrics]),
        'f1': np.mean([m['f1'] for m in fold_val_metrics]),
        'auc': np.mean([m['auc'] for m in fold_val_metrics])
    }

    print("\n=== 所有折的平均性能 ===")
    print("训练集平均性能：")
    print(f"  准确率: {train_metrics_avg['accuracy']:.4f}, 精确率: {train_metrics_avg['precision']:.4f}")
    print(f"  召回率: {train_metrics_avg['recall']:.4f}, F1: {train_metrics_avg['f1']:.4f}, AUC: {train_metrics_avg['auc']:.4f}")
    print("验证集平均性能：")
    print(f"  准确率: {val_metrics_avg['accuracy']:.4f}, 精确率: {val_metrics_avg['precision']:.4f}")
    print(f"  召回率: {val_metrics_avg['recall']:.4f}, F1: {val_metrics_avg['f1']:.4f}, AUC: {val_metrics_avg['auc']:.4f}")
    ...
    print(f"\n=== 本次实验使用的固定超参数 ===")
    print(f"GCN Layer 1 Size: {fixed_best_params['gcn_layer1_size']}")
    print(f"GCN Layer 2 Size: {fixed_best_params['gcn_layer2_size']}")
    print(f"Dense Layer Size: {fixed_best_params['dense_layer_size']}")
    print(f"Dropout Rate: {fixed_best_params['dropout_rate']:.4f}")
    print(f"L2 Regularization: {fixed_best_params['l2_regularization']:.6f}")
    print(f"Learning Rate: {fixed_best_params['learning_rate']:.6f}")
    print(f"Label Smoothing: {fixed_best_params['label_smoothing']:.3f}")
